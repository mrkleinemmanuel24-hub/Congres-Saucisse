...
SPA_INIT.accueil = function() {
  ...
  for (var d = 1; d <= 3; d++) {
    ...
    if (sessions.length > 4) html += '<li style="color:#22B06C;font-weight:600;">+ ' + (sessions.length - 4) + ' autres sessions...</li>';
    html += '</ul></div>';
...
SPA_INIT.intervenants = function() {
  ...
  el.innerHTML = '<div class="section-banner-wrapper"><img src="img/img-speakers.png" alt="Intervenants"><div class="banner-overlay"><span class="banner-title">Nos intervenants</span></div></div><section class="section"><h1 class="section-title"><img src="img/icons/icon-intervenants.png" alt="Intervenants" style="width:48px;height:48px;border-radius:12px;object-fit:contain;vertical-align:middle"> Nos intervenants</h1><p class="section-subtitle">22 experts internationaux.</p><div class="filters" id="interv-theme-filters"></div><div class="grid-3" id="interv-speakers-grid"></div></section>';
  ...
  window.SPA_INTERV = {
    ...
    openSpeaker: function(id) {
      ...
      var html = '<div style="text-align:center;margin-bottom:20px;"><div class="speaker-avatar" style="width:100px;height:100px;font-size:2.2rem;margin:0 auto 12px;">' + s.initiales + '</div><h2 style="font-size:1.3rem;color:#171C22;">' + s.prenom + ' ' + s.nom + '</h2><div style="color:#22B06C;font-weight:600;font-size:.9rem;">' + s.titre + '</div><div style="color:#718096;font-size:.82rem;">' + s.org + '</div></div><div style="background:#F8F9FA;border-radius:12px;padding:16px;margin-bottom:20px;border:1px solid #EDF2F7;"><p style="font-size:.88rem;color:#4A5568;line-height:1.7;">' + s.bio + '</p></div>';
      if (sessions.length > 0) {
        html += '<h3 style="font-size:.95rem;color:#22B06C;margin-bottom:12px;">Sessions (' + sessions.length + ')</h3>';
        sessions.forEach(function(sess) {
          var salle = CONGRES.getSalle(sess.salle);
          html += '<div style="display:flex;gap:12px;padding:10px;background:#F8F9FA;border-radius:10px;margin-bottom:8px;border:1px solid #EDF2F7;"><div style="min-width:60px;text-align:center;font-weight:700;color:#22B06C;font-size:.85rem;">' + sess.debut + '</div><div><div style="font-weight:600;font-size:.88rem;color:#171C22;">' + sess.titre + '</div><div style="font-size:.78rem;color:#718096;">' + dayNames[sess.jour] + ' · ' + (salle ? salle.nom : '') + '</div></div></div>';
        });
      }
...
SPA_INIT.inscription = function() {
  ...
  el.innerHTML = '<div class="section-banner-wrapper"><img src="img/img-inscription.png" alt="Inscription"><div class="banner-overlay"><span class="banner-title">Inscription</span></div></div><section class="section" style="max-width:800px;"><h1 class="section-title"><img src="img/icons/icon-inscription.png" alt="Inscription" style="width:48px;height:48px;border-radius:12px;object-fit:contain;vertical-align:middle"> Inscription</h1><p class="section-subtitle">Reservez votre place au Congres de la Saucisse 2026.</p><div class="stepper"><div class="step-indicator active" data-step="1">1. Infos</div><div class="step-indicator" data-step="2">2. Pass</div><div class="step-indicator" data-step="3">3. Sessions</div><div class="step-indicator" data-step="4">4. Repas</div><div class="step-indicator" data-step="5">5. Hotel</div><div class="step-indicator" data-step="6">6. Recap</div><div class="step-indicator" data-step="7">7. OK</div></div>' +
    '<div class="step-content active" data-step="1"><h2 style="font-size:1.1rem;margin-bottom:20px;">Informations personnelles</h2><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;"><div class="form-group"><label>Prenom *</label><input type="text" id="f-prenom" placeholder="Votre prenom"></div><div class="form-group"><label>Nom *</label><input type="text" id="f-nom" placeholder="Votre nom"></div></div><div class="form-group"><label>Email *</label><input type="email" id="f-email" placeholder="votre@email.com"></div><div class="form-group"><label>Telephone</label><input type="tel" id="f-tel" placeholder="06 12 34 56 78"></div><div class="form-group"><label>Entreprise</label><input type="text" id="f-entreprise" placeholder="Nom entreprise"></div><div class="form-group"><label>Regime alimentaire</label><select id="f-regime" style="width:100%;padding:10px;border-radius:10px;border:1px solid #E2E8F0;background:#FFF;color:#171C22;font-size:.88rem;"><option value="standard">Standard</option><option value="vegetarien">Vegetarien</option><option value="vegan">Vegan</option><option value="sans-gluten">Sans gluten</option><option value="halal">Halal</option></select></div><label style="display:flex;align-items:flex-start;gap:8px;font-size:.82rem;color:#4A5568;margin-top:16px;"><input type="checkbox" id="f-rgpd" style="width:18px;height:18px;accent-color:#2AD783;flex-shrink:0;margin-top:2px;"> J\'accepte le traitement de mes donnees (RGPD) *</label><div style="text-align:right;margin-top:20px;"><button class="btn btn-green" onclick="SPA_REG.nextStep(2)">Suivant</button></div></div>' +
  ...
  window.SPA_REG = {
    ...
    initHotels: function() { var self=this,html=''; CONGRES.HOTELS.forEach(function(h){var sel=self.reg.hotel===h.id;html+='<div class="hotel-option'+(sel?' selected':'')+'" onclick="SPA_REG.selectHotel('+h.id+')" data-hotel="'+h.id+'"><div style="flex:1;"><strong>'+h.nom+'</strong><br><span style="font-size:.82rem;color:#718096;">'+h.distance+'</span></div><div style="text-align:right;"><div style="font-size:1.1rem;font-weight:800;color:#22B06C;">'+h.prix+' &euro;</div><div style="font-size:.75rem;color:#718096;">/nuit</div></div></div>';}); document.getElementById('ins-hotel-choices').innerHTML=html; },
...
SPA_INIT.infos = function() {
  ...
  el.innerHTML = '<div class="section-banner-wrapper"><img src="img/hero-strasbourg.png" alt="Strasbourg"><div class="banner-overlay"><span class="banner-title">Infos pratiques</span></div></div><section class="section"><h1 class="section-title">Infos pratiques</h1><p class="section-subtitle">Tout pour preparer votre venue.</p><div class="gallery-grid"><div class="gallery-card"><img src="img/img-strasbourg-cathedral.png" alt="Cathedrale"><div class="caption">Cathedrale Notre-Dame</div></div><div class="gallery-card"><img src="img/img-petite-france.png" alt="Petite France"><div class="caption">Petite France</div></div><div class="gallery-card"><img src="img/img-tram.png" alt="Tram"><div class="caption">Tram CTS</div></div></div><div class="info-section"><h2><img src="img/icons/icon-plan.png" alt="" style="width:48px;height:48px;border-radius:12px;object-fit:contain;vertical-align:middle"> Plan du quartier</h2><div id="spa-map" class="map-container" style="margin-bottom:12px;"></div></div><div class="info-section"><h2><img src="img/icons/icon-transport.png" alt="" style="width:48px;height:48px;border-radius:12px;object-fit:contain;vertical-align:middle"> Venir a Strasbourg</h2><div class="transport-grid"><div class="transport-card"><h3>TGV</h3><p><strong>Paris → Strasbourg : 1h46</strong><br>Tram B arret Wacken (20 min).</p></div><div class="transport-card"><h3>Avion</h3><p><strong>Aeroport Entzheim (SXB)</strong><br>Navette train 9 min.</p></div><div class="transport-card"><h3>Voiture</h3><p><strong>A4 depuis Paris (4h30)</strong><br>Parking Wacken 800 places.</p></div><div class="transport-card"><h3>Tram</h3><p>Lignes B et E, arret <strong>Wacken</strong>.</p></div></div></div><div class="info-section"><h2><img src="img/icons/icon-restaurant.png" alt="" style="width:48px;height:48px;border-radius:12px;object-fit:contain;vertical-align:middle"> Guide gourmand</h2><div class="gallery-grid" style="margin-bottom:20px;"><div class="gallery-card"><img src="img/img-choucroute.png" alt="Choucroute"><div class="caption">Choucroute garnie</div></div><div class="gallery-card"><img src="img/img-flammekueche.png" alt="Flammekueche"><div class="caption">Flammekueche</div></div></div><div class="resto-grid"><div class="resto-card"><h4>Maison Kammerzell</h4><div class="type">Winstub historique</div><p>Cuisine alsacienne, Place de la Cathedrale.</p></div><div class="resto-card"><h4>Chez Yvonne</h4><div class="type">Winstub traditionnelle</div><p>Saucisses grillees, choucroute royale.</p></div><div class="resto-card"><h4>La Corde a Linge</h4><div class="type">Flammekueche creative</div><p>Petite France.</p></div></div></div><div class="info-section"><h2><img src="img/icons/icon-urgence.png" alt="" style="width:48px;height:48px;border-radius:12px;object-fit:contain;vertical-align:middle"> Numeros utiles</h2><div class="emergency-grid"><div class="emergency-card"><div class="number">15</div><div class="label">SAMU</div></div><div class="emergency-card"><div class="number">17</div><div class="label">Police</div></div><div class="emergency-card"><div class="number">18</div><div class="label">Pompiers</div></div><div class="emergency-card"><div class="number">112</div><div class="label">Urgences EU</div></div></div></div></section>';
